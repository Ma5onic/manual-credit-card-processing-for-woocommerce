...
class MWC_CC_Processing {
    ...
    public function __construct() {
        ...
        $this->local_assets_path = str_replace( array( 'http:', 'https:' ), '', untrailingslashit( plugins_url( '/', realpath('../moco-woocommerce-manual-cc-processing.php') ) ) ) . '/manual-credit-card-processing-for-woocommerce/assets/';
    ...
    public function set_shipping_address($order){
        ...
        if($legacy) {
            ...
            $shipping = array(
                ...
                'PostalCode' => (!($order->get_shipping_postcode())) ? $order->get_billing_postcode() : $order->get_shipping_postcode(),
                'City' => (!($order->get_shipping_city())) ? $order->get_billing_city() : $order->get_shipping_city(),
                'Address1' => (!($order->get_shipping_address_1())) ? $order->get_billing_address_1() : $order->get_shipping_address_1(),
                'Address2' => (!($order->get_shipping_address_2())) ? $order->get_billing_address_2() : $order->get_shipping_address_2()
            );
        }
...
<p class="form-field form-field-wide">
    ...
    <label><input type="checkbox" class="" style="width:0" name="guest_to_customer" id="guest_to_customer" disabled> <i>Customer must be set to "Guest"</i>.</label>
    <label><strong>PRO Version Only</strong></label>
</p>
CCC;
    ...
    function _create_customer( $post_id, $post, $update ) {
        ...
        if ( isset( $_POST['guest_to_customer'] ) ) {
            if( (isset($_POST['customer_user']) && trim($_POST['customer_user']) == "") || !isset($_POST['customer_user'])) {
                $_SESSION['mwc_cc_processing_error'] = 'Creating users from guests is a pro feature.';
            }
        }
    }
