...
if(!empty($_GET['post']) && !in_array($_GET['wc-ajax'],array('get_refreshed_fragments'))) {
    ...
    if($order->needs_payment()){
      try {
          ...
          wp_set_current_user((version_compare( WC_VERSION, '3.0.0', '<' )) ? $order->user_id : $order->get_user_id());

          $payment_gateway = wc_get_payment_gateway_by_order($order);
          $order_id = (!is_object($order)) ? absint($order) :(version_compare( WC_VERSION, '3.0.0', '<' )) ? $order->id : $order->get_id();
          ...
          WC()->cart = new WC_Cart();
          WC()->customer = new WC_Customer((version_compare( WC_VERSION, '3.0.0', '<' )) ? $order->user_id : $order->get_user_id());
          ...
          foreach ($manualLineItems as $li) {
              ...
              if(version_compare( WC_VERSION, '3.0.0', '<' )){
                  ...
                  wc_update_product_stock($_product, 10 * $productQty);
                  WC()->cart->add_to_cart((int) $productId, $productQty, $variationId);
                  wc_update_product_stock($_product, 10 * $productQty);
              }
          }
              ...
              <div class="wc-backbone-modal">
                  <div class="wc-backbone-modal-content">
                      <section class="wc-backbone-modal-main" role="main" style="padding-bottom: 0">
                          ...
                          <article>
                              <div class="shipping">
                                  Auto Calculated Shipping Rates are a PRO feature.
                              </div>
                          </article>
                      </section>
                  </div>
